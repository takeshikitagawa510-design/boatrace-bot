...
if os.path.exists(CACHE_FILE):
    try:
        ...
        print(
            f"📦 過去の通知済みデータ ({len(notified_races)}件) を読み込みました。"
        )
            ...
            if kakusei_race_id not in notified_races:
                ...
                if is_triggered:
                    ...
                    send_discord_embed(
                        title=(
                            "🚨 機力覚醒シグナル検知！"
                            f" 【{venue_japanese} {rno_str}R】"
                        ),
                        description=(
                            "対象艇の舟足・機力が大幅覚醒！高配当ターゲットレースです。"
                        ),
                        fields=fields,
                        color=0xFF0055,
