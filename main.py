...
def main():
    ...
    for jcd in range(1, 25):
        jcd_str = f"{jcd:02d}"
        if jcd_str in EXCLUDE_VENUES or jcd_str not in VENUE_NAMES: continue
        ...
        for rno in range(1, 13):
            try:
                ...
                for w, tbody in enumerate(tbodys[:6], 1):
                    ...
                    racers[w] = {'name': name, 'sec_st': sec_st, 'has_sec': len(t_sts) > 0}

                if any(not racers[w]['has_sec'] for w in range(1, 7)): continue
                r1 = racers[1]
                ...
                if is_valid_manshu:
                    ...
                    if it > 0:
                        m_total = sum(scoring_dict.get('manshu_overall', {}).get(racers[wk]['name'], 0) for wk in range(2, 7))
                        final_score = it + m_total + 30
                        if final_score >= 150:
                            manshu_races.append({
                                'v': VENUE_NAMES[jcd_str], 
                                'r': rno, 
                                's': final_score, 
